...
cy.on("cxttap", "edge", function (evt) {
    ...
    if (floatDiv) {
        ...
        document.addEventListener('contextmenu', function onMouseUpdate(e) {
            ...
            let x = e.pageX
            let y = e.pageY
            document.getElementById('optionsEdge').style.top = String(y) + "px"
            document.getElementById('optionsEdge').style.left = String(x) + "px"
            document.getElementById('optionsEdge').style.display = 'block'
        })
    }
})

function displayOptionsEdgeOff(optionAction) {
    if (optionAction) {
        let valor = document.getElementById('edge').value
        //console.log(valor);
        cy.$("#" + String(idModify)).style({'label': valor})
    }
    document.getElementById('optionsEdge').style.display = 'none'
    floatDiv = true
}
...
function resetTudo(){
    ...
    contNode = 0 
    document.getElementById("floyd-table").innerHTML = "";
}
...
function estadosFROYD(op) {
    if(op){
        ...
        if(estado.valor == matrizEstado.length && estadoCentral.valor + 1 < EstadoCompleto.length)
        { 
            ...
            cy.edges().map(arestaDaTela => {
                ...
                for (v = cy.nodes().length-1; v != 0; v = parent[v]) {
                    ...
                    if(u == source && v == target){
                        ...
                        cy.$("#" + String(source) + String(target)).style({label: peso ? peso : 0,"line-color": "#fbc02d"})
                    }
                }
            })
        ...
        if(estado.valor == matrizEstado.length)
        {
            estadoAnt = EstadoCompleto[estadoCentral.valor].matrizEstado;
            matrizEstado = EstadoCompleto[estadoCentral.valor].matrizEstado;
            estado.valor-- ;
            parent = EstadoCompleto[estadoCentral.valor].parent;
            cy.edges().map(arestaDaTela => {
                ...
                for (v = cy.nodes().length-1; v != 0; v = parent[v]) {
                    ...
                    if(u == source && v == target){
                        let peso = parseInt(cy.$("#" + String(source) + String(target)).style().label) + EstadoCompleto[estadoCentral.valor].Flow 
                        cy.$("#" + String(source) + String(target)).style({label: peso,"line-color": "#fbc02d"})
                    }
                }
            })
        }
